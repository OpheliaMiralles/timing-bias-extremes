...
class GEV_reparametrized_rp(ScipyDistribution):
    params_names = ("loc", "m", "shape", "r")
    base_module = genextreme

    def __init__(self, loc=0.0, m=200, shape=0.0, r=1.0):
        super(GEV_reparametrized_rp, self).__init__(loc, m, shape, ConstantParameter(r))

    def _to_scipy_args(self, loc=None, m=None, shape=None):
        loc = ifnone(loc, self.loc())
        shape = ifnone(shape, self.shape())
        r = float(self.r())
        rp = ifnone(m, self.m())
        scale = (r - loc) * shape / ((-np.log(1 - 1/rp)) ** (-shape) - 1) if shape != 0 else (r - loc) / (-(np.log(-np.log(1 - 1/rp))))
        if shape is not None:
            shape = -shape
        return {
            "c": ifnone(shape, -self.shape()),
